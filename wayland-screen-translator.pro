...
TARGET = wayland-screen-translator
TEMPLATE = app
CONFIG += c++17
...
LIBS += -L$$DEPS_DIR/lib
LIBS += -lhunspell-1.7 -lleptonica -ltesseract

win32{
  LIBS += -lUser32
}
linux{
  packagesExist(Qt5X11Extras) {
    QT += x11extras
    LIBS += -lX11
    DEFINES += ST_X11_GLOBAL_SHORTCUTS
  } else {
    message("Building without X11 global shortcut support")
  }
}
...
DEFINES += VERSION="$$VER"
VERSION = $$VER.0
QMAKE_TARGET_COMPANY = sithond
QMAKE_TARGET_PRODUCT = Wayland Screen Translator
QMAKE_TARGET_COPYRIGHT = Maintained by sithond
...
OTHER_FILES += \
  ...
  version.json \
  updates.json \
  share/wayland_portal_capture.py
...
linux {
    ...
    shortcuts.files = $$PWD/share/wayland-screen-translator.desktop
    shortcuts.path = $$PREFIX/share/applications/
    pixmaps.files += $$PWD/share/images/wayland-screen-translator.png
    pixmaps.path = $$PREFIX/share/icons/hicolor/128x128/apps/
    waylandhelper.files = $$PWD/share/wayland_portal_capture.py
    waylandhelper.path = $$PREFIX/share/screen-translator/

    INSTALLS += target shortcuts pixmaps waylandhelper
}
win32 {
    RC_ICONS = $$PWD/share/images/icon.ico
    target.path = /
