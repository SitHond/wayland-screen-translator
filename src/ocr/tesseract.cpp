...
static Pix *prepareImage(const QImage &image)
{
...

  {
    ...
    // Top
    const auto otsuWidth = pixGetWidth(otsu);
    const auto otsuHeight = pixGetHeight(otsu);

    auto avg = pixAverageOnLine(otsu, 0, 0, otsuWidth - 1, 0, 1);
    // Bottom
    avg += pixAverageOnLine(otsu, 0, otsuHeight - 1, otsuWidth - 1, otsuHeight - 1, 1);
    // Left
    avg += pixAverageOnLine(otsu, 0, 0, 0, otsuHeight - 1, 1);
    // Right
    avg += pixAverageOnLine(otsu, otsuWidth - 1, 0, otsuWidth - 1, otsuHeight - 1, 1);
    avg /= 4.0f;
